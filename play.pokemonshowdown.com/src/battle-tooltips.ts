...
class ModifiableValue {
	...
	constructor(battle: Battle, pokemon: Pokemon, serverPokemon: ServerPokemon) {
		...
		this.weatherName =
			battle.weather === "snow"
				? "Snow"
				: this.battle.dex.moves.get(battle.weather).exists
...
class BattleTooltips {
	...
	showPokemonTooltip(
		...
		if (pokemon.fainted) {
			...
			text +=
				"<p><small>HP:</small> " +
				Pokemon.getHPText(pokemon, this.battle.reportExactHP) +
				exacthp +
				(pokemon.status
					? ' <span class="status ' +
					  pokemon.status +
	...
	showFieldTooltip() {
		...
		for (const side of this.battle.sides) {
			...
			buf += `<td><p class="tooltip-section"><strong>${BattleLog.escapeHTML(
				side.name
			...
			if (item !== "utilityumbrella") {
				...
				if (weather === "raindance" || weather === "primordialsea") {
					...
					if (
						...
						// Check for the actual highest stat
						for (let key of ["def", "spa", "spd", "spe"]) {
							if (stats[key] > maxStat) {
								high = key;
								maxStat = stats[key];
							}
						}

						// Increase the highest stat by 50%
						stats[high] *= 1.5;
					}
					if (ability === "tundracore" && weather === "snow") {
						let high = "atk"; // Assume "atk" is the highest by default
						let maxStat = stats.atk; // Store its value
					...
					if (ability === "desertheart" && weather === "sandstorm") {
						let high = "atk"; // Assume "atk" is the highest by default
						let maxStat = stats.atk; // Store its value
			...
			if (move.id === "bodycount") {
				value.set(
					...
					pokemon.side.faintCounter > 0
						? `${pokemon.side.faintCounter} teammate${
								pokemon.side.faintCounter > 1 ? "s" : ""
						  } KOed`
						: undefined
				);
			}
					...
					if (this.battle.rules["Frantic Fusions Mod"]) {
						...
						if (
							...
							abilityData.possibilities = Array.from(
								new Set(
									abilityData.possibilities.concat(
										Object.values(fusionSpecies.abilities)
									)
								)
							);
						}
