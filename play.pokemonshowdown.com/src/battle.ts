	...
	effectiveAbility(serverPokemon?: ServerPokemon) {
		...
		if (
			...
			(!ability.flags["cantsuppress"] &&
				(this.side.battle.ngasActive() || this.volatiles["gastroacid"]))
		) {
			return "";
		}
		return ability.name;
...
export class Side {
	...
	addPokemon(name: string, ident: string, details: string, replaceSlot = -1) {
		const oldPokemon =
			replaceSlot >= 0 ? this.pokemon[replaceSlot] : undefined;
...
export class Battle {
	...
	runMinor(args: Args, kwArgs: KWArgs, nextArgs?: Args, nextKwargs?: KWArgs) {
		...
		switch (args[0]) {
			...
			case "-heal": {
				let poke = this.getPokemon(
					args[1],
					Dex.getEffect(kwArgs.from).id === "revivalblessing"
				)!;
			...
			case "-item": {
				...
				if (!poke) {
					if (effect.id === "frisk") {
						const possibleTargets = ofpoke!.side.foe.active.filter(
							(p) => p !== null
						);
			...
			case "-terastallize": {
				...
				if (poke.speciesForme.startsWith("Morpeko")) {
					...
					poke.details = poke.details.replace(
						"Morpeko",
						poke.speciesForme
					);
			...
			case "-end": {
				...
				if (
					kwArgs.silent &&
					!(effect.id === "protosynthesis" || effect.id === "quarkdrive")
				) {
					// do nothing
				} else {
					switch (effect.id) {
		...
		switch (args[0]) {
			...
			case "showteam": {
				...
				for (const set of team) {
					const details =
						set.species +
						(!set.level || set.level === 100 ? "" : ", L" + set.level) +
						(!set.gender || set.gender === "N" ? "" : ", " + set.gender) +
						(set.shiny ? ", shiny" : "");
